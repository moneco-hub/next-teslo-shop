...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  useEffect(() => {
    const subscription = watch((value, { name }) => {
      if (name === "title") {
        const newSlug = value.title?.trim().replaceAll(" ", "_").replaceAll("'", "") || "";
        setValue("slug", newSlug.toLowerCase(), { shouldValidate: true });
      }
    });
  ...
  const onSubmit = async (form: FormData) => {
    if (form.images.length < 2) return alert("Provide at least two images");
    setIsSaving(true);
    ...
    <AdminLayout title={"Product"} subTitle={`Editing: ${product.title}`} icon={<DriveFileRenameOutline />}>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Box display="flex" justifyContent="end" sx={{ mb: 1 }}>
          <Button disabled={isSaving} color="secondary" startIcon={<SaveOutlined />} sx={{ width: "150px" }} type="submit">
            Save
          </Button>
        </Box>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <FormControl sx={{ mb: 1 }}>
              ...
              <RadioGroup
                ...
                onChange={({ target: { value } }) => setValue("type", value, { shouldValidate: true })}
              >
          ...
          <Grid item xs={12} sm={6}>
            ...
            <TextField
              ...
              value={newTagValue}
              onChange={(e) => setNewTagValue(e.target.value as string)}
